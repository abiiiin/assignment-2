...
pub mod pallet {
    ...
    pub trait Config: frame_system::Config {
        ...
        // Add TokenReserved constant
        #[pallet::constant]
        type TokenReserved: Get<u32>;
    }
    ...
    pub enum Error<T> {
        ...
        NotEnoughBalance,
        /// Failed to converse integer to balance.
        ConversionError,
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create_kitty(origin: OriginFor<T>) -> DispatchResult {
            ...
            let rb = T::TokenReserved::get().try_into().map_err(|_e| Error::<T>::ConversionError)?;

            // Check the buyer has enough free balance
			ensure!(T::Currency::free_balance(&sender) >= rb, <Error<T>>::NotEnoughBalance);

            let kitty_id = Self::mint(&sender, None, None)?; // <- add this line

            // Reserve the amount from creator
            T::Currency::reserve(&sender, rb.clone())?;
